...
public class AnnotatedQueryTests
{
    ...
    [Theory]
    [MemberData(nameof(TestUtilities.TestContextData), MemberType = typeof(TestUtilities))]
    public void WhereIfTrueCondition(string testContextName, TestContext testContext)
    {
        var query = testContext.Query;
        var filterByMovie = true;
        var cypherQuery = query.AsAnnotatedQuery()
            //.Where((MovieNode movie) => movie.Title == "Grey's Anatomy")
            .WhereIf(filterByMovie, (MovieNode movie) => movie.Year == 2017)
            .AsCypherQuery()
            .Query;

        var actual = cypherQuery.QueryText;
        var expected = filterByMovie ? $"WHERE (movie.Year = ${cypherQuery.QueryParameters.Last().Key})" : "";
    ...
    [Theory]
    [MemberData(nameof(TestUtilities.TestContextData), MemberType = typeof(TestUtilities))]
    public void WhereIfFalseCondition(string testContextName, TestContext testContext)
    {
        var query = testContext.Query;
        var filterByMovie = false;
        var cypherQuery = query.AsAnnotatedQuery()
            //.Where((MovieNode movie) => movie.Title == "Grey's Anatomy")
            .WhereIf(filterByMovie, (MovieNode movie) => movie.Year == 2017)
            .AsCypherQuery()
